...
export default function EditCategoryPage() {
  ...
  const [form, setForm] = useState({ name: "", slug: "", description: "", icon: "", sort: 0 });

  useEffect(() => {
    fetch(`/api/admin/categories`).then(r => r.json()).then((cats) => {
      const c = cats.find((x: any) => x.id === id);
      if (c) setForm({ name: c.name, slug: c.slug, description: c.description || "", icon: c.icon || "", sort: c.sort || 0 });
      setFetching(false);
    });
  }, [id]);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-display font-bold text-[var(--text-primary)]">Edit Kategori</h1>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-lg">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="text" value={form.icon} onChange={(e) => setForm((p) => ({ ...p, icon: e.target.value }))} className="input" maxLength={10} />
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">Deskripsi</label>
          <textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} className="input min-h-[80px]" rows={3} />
        </div>
