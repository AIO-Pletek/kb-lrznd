...
export default function EditTagPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/tags`).then(r => r.json()).then((tags) => {
      const t = tags.find((x: any) => x.id === id);
      if (t) setForm({ name: t.name, slug: t.slug });
      setFetching(false);
    });
  }, [id]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/tags/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-display font-bold text-[var(--text-primary)]">Edit Tag</h1>
        </div>
